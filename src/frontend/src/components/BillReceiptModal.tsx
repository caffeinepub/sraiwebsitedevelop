...
export function BillReceiptModal({ bill, open, onOpenChange }: Props) {
  ...
  const buildWhatsApp = () => {
    ...
    if (bill.lensRight && Object.values(bill.lensRight).some((v) => v)) {
      lensLines.push("", "🔍 Right Eye:");
      lensLines.push(
        `SPH: ${bill.lensRight.sph} | CYL: ${bill.lensRight.cyl} | AXIS: ${bill.lensRight.axis} | ADD: ${bill.lensRight.add} | Near: ${bill.lensRight.nearVision}`,
      );
    }
    if (bill.lensLeft && Object.values(bill.lensLeft).some((v) => v)) {
      lensLines.push("Left Eye:");
      lensLines.push(
        `SPH: ${bill.lensLeft.sph} | CYL: ${bill.lensLeft.cyl} | AXIS: ${bill.lensLeft.axis} | ADD: ${bill.lensLeft.add} | Near: ${bill.lensLeft.nearVision}`,
      );
    }
    ...
    const lines = [
      ...
      bill.additionalDiscount > 0
        ? `🏷️ Additional Discount: ₹${bill.additionalDiscount.toFixed(2)}`
        : "",
      `✅ Grand Total: ₹${bill.grandTotal.toFixed(2)}`,
      `💵 Advance: ₹${bill.advance.toFixed(2)}`,
      `⚠️ Dues: ₹${bill.dues.toFixed(2)}`,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
                      ...
                      <div className="flex items-center gap-2">
                        {p.photo && (
                          <img
                            src={p.photo}
                            alt={p.name}
                            className="w-7 h-7 rounded object-cover shrink-0 border"
                          />
                        )}
        ...
        {/* Action Buttons */}
        <div className="no-print px-5 pb-5 pt-2 flex gap-2">
          <Button
